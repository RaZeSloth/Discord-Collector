const { ReactionRoleManager } = require('discord-collector')
...
client.on("messageCreate", async (message) => {
    ...
    if (message.content.startsWith('>createReactionRole')) {
        ...
        message.reply('Done').then(m => setTimeout((async () => { await m.delete() }), 500));
    }
    else if (message.content.startsWith('>deleteReactionRole')) {
        const emoji = args[0];
        if (!emoji) {
            message.reply('You need use a valid emoji.').then(m => setTimeout((async () => { await m.delete() }), 1000)); return;
        }
